# TacticalFramework — Unreal Engine 5 Plugin
...
### 1. Enable the Plugin
Copy the `Plugins/TacticalFramework` folder into your project's `Plugins/` directory. Restart the editor.
...
```
┌─────────────────────────────────────────────────────────────┐
│                    PayloadMissionManager                     │
│  State Machine: NotStarted → InProgress → Success/Failed    │
│  Tracks: Targets, Attempts, Timer, Damage Summary           │
└──────────┬──────────────────────────────────┬───────────────┘
           │ discovers                        │ spawns via
           ▼                                  ▼
┌──────────────────┐              ┌───────────────────────────┐
│   TargetActor    │              │ PayloadAttachmentComponent│
│ ┌──────────────┐ │              │ (attach to ANY pawn)      │
│ │ Damagable    │ │              │                           │
│ │ Component    │ │              │  spawns & attaches        │
│ ├──────────────┤ │              │         │                 │
│ │ Behavior     │ │              │         ▼                 │
│ │ Component    │ │              │    ┌─────────┐            │
│ ├──────────────┤ │              │    │ Payload │            │
